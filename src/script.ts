document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 2. Language Logic ---
    const langSelect = document.getElementById('lang-switch') as HTMLSelectElement | null;
    ...
    function updateThemeUI(mode: string, actualTheme: string) {
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', (event) => {
            const targetElement = event.currentTarget as HTMLAnchorElement | null;
    ...
    if (versionBadge) {
        ...
        .then((data: { tag_name?: string; prerelease?: boolean }) => {
